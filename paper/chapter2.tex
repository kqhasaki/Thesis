...
$$
其中$K(x)$为核函数，$h \rightarrow 0$是带宽，关于带宽的选择（Weidong Liu，2020）。
...
\begin{table}[H]%%%%%%开始表格
    ...
    \toprule%第一道横线 表头
    算法2.2 使用替代变量估计方法求解最小绝对值回归问题\\
    \midrule%第二道横线 符号+解释+单位 中间用&隔开
    输入：$Y$和$\bm{X}$的样本$\bm{Y} = (Y_1, Y2, ..., Y_n)$，$\bm{X} = (\bm{X}^T_1, \bm{X}^T_2, ..., \bm{X}^T_n)$，
    迭代次数$T$，核函数$K$，带宽$h_t(t = 1, ..., T)。$
    \\
    初始化：给出初始相合估计量，$\hat{\bm{\beta}}^{(0)} $
    \\
    对于$t = 1, ..., t$：
    \\
        1. 计算$\hat{f}^{(t)}(0)$，
        $$
        \hat{f}^{(t)}(0) = \frac1{nh}\sum_{i=1}^{n}K(\frac{Y_i - \bm{X}_i^T\hat{\bm{\beta}}^{(t-1)}}{h_t})
        $$
    \\
    ...
    \\
        3. 计算$\hat{\bm{\beta}}^{(t)}$,
        $$
        \hat{\bm{\beta}}^{(t)} = \underset{\bm{\beta} \in \mathbb{R}^{p+1}}{\operatorname{art\ min}}
        \frac1{n}\sum_{i=1}^n (\tilde{Y}_i - \bm{X}_i^T\bm{\beta})^2
        $$
    \\
    输出：$\bm{\beta}^{(T)}$
...
\subsubsection{优化高维最小绝对值回归}
在高维自变量需要进行筛选的情形下，我们只需在\eqref{beta1-original}加入惩罚项即可，例如我们使用$L_1$正则化，那么
\begin{equation}
...
\subsection{模拟实验}
本章前面两节介绍了聚类——迭代拆解算法和一种基于替代变量的估计算法，
...
设定模拟数据来自下面的模型：
\begin{equation*}
    {Y}_i = \bm{X}_i^T \bm{\beta}_* + e_i, i = 1, 2, ..., n
\end{equation*}
其中$\bm{X}_i = (1, X_{i,1}, ..., X_{i, p-1})$为$p$维随机向量，
$X_{i,1}, ..., X_{i,p-1}$均来自多元正态分布$N(\bm{0}, \bm{\Sigma})$，
协方差矩阵$\bm{\Sigma}_{ij} = 0.5^{|i-j|},\ 1\leq i, j \leq p-1$，
我们设置两组$p, n$的组合用来测试算法性能，
\begin{equation*}
    \begin{split}
        & \text{数组组A：} n = 10^6, \ p \in \{ 50, 100, 150, 200, 250, 300, 350, 400, 450, 500, 600, 700, 800, 1000, 1500\} \\
        & \text{数组组B：} n \in \{ 4,6,8,10,12,14,16\} \times 10^5,\ p \in \{50, 100, 500, 1000 \}
    \end{split}
\end{equation*}
数据组A主要用来测试在给定样本量下，数据维数对算法的性能影响；
数据组B主要用来测试不同样本量下算法的表现。

为了模拟高维线性模型的应用场景，我们给$\bm{\beta}_*$设置一定的稀疏性。记$s = |\bm{\beta} |_0$为其非零维个数，
取$\bm{\beta}^* = (\frac{10}{s}, \frac{20}{s}, ..., \frac{10(s-1)}{s}, 10, 0, ..., 0)$，

算法2.1的每步带宽依赖于稀疏性，参考weidong liu等，这里给出
$$
...
并且着重观察算法经历的迭代次数、消耗时间、和在重尾分布下的表现。
...
\subsection{本章小结}
最小绝对值回归已经广泛应用在高维宏观经济数据的处理中，并且在应用中存在一定的性能问题。
