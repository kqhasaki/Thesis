...
图\ref{compare-rotate}中展示了同一组数据分别采用PCA-$L_1$、$L_1$-PCA以及$L_2$-PCA得到的因子载荷矩阵热力图。
为便于分析，我们已将因子载荷矩阵进行了最大方差旋转，可以看出三种不同的主成分分析方法得到的主成分是很不一样的。

前文已经通过实证研究发现，$L_1$-PCA可以作为因子模型的主成分估计，
并且在扩散指数模型中拥有良好的预测表现。
这里我们同样基于国内月度宏观经济数据对PCA-$L_1$进行测试，将得到的$L_1$主成分
